...
# sys.path.append('../')
sys.path.append('../DB')
from SRC.WEB import YGGetWebData
from SRC.Database import btsForDashin,MakeDB
import time
import linecache
import pandas as pd
import datetime
...
#         self.initConfigSet()
        CloseData = self.getPrice(self.ForeignerDB,self.ClosePriceTable,code)
        date_fmt = '%Y-%m-%d'
        Date = self.getColumns(self.ForeignerDB,self.ClosePriceTable)
